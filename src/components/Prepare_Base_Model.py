...
class BaseModel:
    ...
    def get_base_model(self):
        if self.config.base_model_path.exists():
            print(f"Base model already exists at {self.config.base_model_path}, loading...")
    ...
    def update_base_model(self):
        if self.config.updated_base_model_path.exists():
            print(f"Updated model already exists at {self.config.updated_base_model_path}, loading...")
            self.full_model = tf.keras.models.load_model(self.config.updated_base_model_path)
            return
